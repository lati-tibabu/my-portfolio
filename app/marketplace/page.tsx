      ...
      <section className="px-6 pt-24 pb-12">
        <div className="max-w-[1200px] mx-auto space-y-4">
          ...
          <p className="text-[16px] text-[var(--color-on-surface-variant)] max-w-[700px]">
            Discover apps, themes, templates, and tools with pricing,
            previews, requirements, and direct links to get started.
          </p>
        </div>
      </section>

      <section className="px-6 pb-20">
        {marketplaceItems.length === 0 && <CollectionEmptyState title="Products are on the way" description="Looking for a digital tool or resource? Get in touch to discuss products or a custom solution." />}
            ...
            >
              <Link href={`/marketplace/${item.slug}`} className="block">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  {item.version && <span className="tag-chip">v {item.version}</span>}
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {item.license && <span className="tag-chip">{item.license}</span>}
              </div>
