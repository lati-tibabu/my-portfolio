  ...
  return (
    <header className="site-header sticky top-0 z-50">
      <div className="mx-auto flex h-20 max-w-[1280px] items-center justify-between gap-5 px-6">
        ...
        <nav aria-label="Main navigation" className="header-links hidden items-center gap-1 md:flex">
        ...
        <div className="flex items-center gap-3">
          <ThemeToggle />
          {isAdminRoute && adminEmail && <button type="button" onClick={handleAdminSignOut} className="hidden rounded-full border border-[var(--color-surface-border)] p-3 md:inline-flex" aria-label={`Sign out ${adminName}`}><Icon name="power" size={17} /></button>}
          ...
          <button ref={menuButtonRef} type="button" aria-expanded={mobileOpen} aria-controls="mobile-navigation" className="flex h-11 w-11 items-center justify-center rounded-full border border-[var(--color-surface-border)] md:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-label={mobileOpen ? "Close menu" : "Open menu"}><Icon name={mobileOpen ? "close" : "menu"} size={22} /></button>
        </div>
      </div>
      {mobileOpen && <nav id="mobile-navigation" aria-label="Mobile navigation" className="max-h-[calc(100dvh-5rem)] overflow-y-auto border-t border-[var(--color-surface-border)] bg-[var(--color-background)] px-6 py-5 md:hidden">
